...
private:
    ...
    inline auto shuffle(M1 & X, M2 & y, T & r_engine)
        -> DSOVK &
    {
...
#ifdef WRAP_PYTHON
    auto py_predict(const pybind11::array_t<double> & X, int nt, long int th)
...
auto fit(const M1 & X, const M2 & y)
    -> DSOVK &
    // Precondition: X is shuffled
{
    set_d(X.rows());
    set_p(y.rows());
    set_batch(std::min(get_batch(), X.cols()));
    ...
    Eigen::ConjugateGradient<DecomposableLinOp<Eigen::MatrixXd,
                                               Eigen::MatrixXd>,
    ...
    for (long int t = 0; t < T; ++t) {
        ...
        // condition_mat(cond_operator.get_Lhs(), get_nu());
        // cond_operator.get_Rhs().noalias() =
        //     phi_w.get_Rhs() *
        //     phi_w.get_Rhs().transpose();
        // preconditioner.compute(cond_operator);
        // update = preconditioner.solveWithGuess(coef_seg, coef_seg);
        update = coef_seg;
...
#ifdef WRAP_PYTHON
    auto py_fit(const pybind11::array_t<double> & X,
