...
p = 10
print np.linalg.pinv(.000001 * np.eye(p) + .999999 * np.ones((p, p)))
# print np.linalg.eigvals(1. * np.eye(p) - .1 * np.ones((p, p)))

# print scode(10).shape
# print np.inner(scode(10)[:, 1], scode(10)[:, 6])
# print - 1. / 9.
...
# C = InverseScaling(1., 1e-1)

# # for i in xrange(T):
# estimator = DSOVK(A, B, C, lbda, 1, batch, block, 0, cond)
# ts = time.time()
# estimator.fit(X, y)
# te = time.time()
